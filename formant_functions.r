...
plot.vowels <- function(m, param1, param2, group.by='phone', meas=NULL, mf=NULL, limits='global', size_param=FALSE, 
    inches=FALSE, manual.xlim=FALSE, manual.ylim=FALSE, xlab=NULL, ylab=NULL, do.ellipses=TRUE, fill=FALSE, do.text=TRUE, 
    noplot=FALSE, subject=NULL, main='', draw.zero=TRUE, replace.underscores=TRUE, rename.axes=TRUE, levels.to.highlight=NULL, 
    label.big.circles=NULL, pch=19, col.override=NULL, col.override2=NULL, overplot=FALSE, rainbow_multiplier=1.2, plot_points=TRUE){
...
findCovarianceMatrices <- function(data, formants=c('F1','F2','F3','F4','F5'), parameters=NULL, measurements=c(0.325), write.to.file=FALSE, filename=NULL, language='French', vowel_means=all.means, normalized=TRUE, pass_id=NULL, target_phones=NULL){
...
critical.bandwidth <- function(z){
    52548/(z^2-52.56*z+690.39)
}



schwartz_F2_prime <- function(F2,F3,F4,units='bark'){
    ...
    c1 <- ifelse(F3-F2>3.5, 0,
            ifelse(F3-F2>2.5, 3.5-(F3-F2),
              ifelse(F4-F2>3.5, 1,
                ifelse(F4-F3>F3-F2, 1, 2))))

    c2 <- ifelse(c1<=1, 1, 0)
    c3 <- ifelse(c1<=1, c1/2, 1)
    c4 <- ifelse(c1>1, 0.5, 0)

    F2_prime <- (c2*F2 + c3*F3 + c4*F4) / (c2 + c3 + c4)
...
#     }
#     #print(schwartz)
#     F2_prime <- (schwartz$c2*F2 + schwartz$c3*F3 + schwartz$c4*F4) / (schwartz$c2+schwartz$c3+schwartz$c4)
#     if (units=='hertz'){
#         hertz(F2_prime)
#     }else{
#         F2_prime
...
# schwartz_F2_prime2 <- function(F2,F3,F4,units='bark'){
#     if (units=='hertz'){
#         F2 <- bark(F2)
#         F3 <- bark(F3)
#         F4 <- bark(F4)
#     }
#     if (F3-F2>3.5){                         #F3 is high enough to be irrelevant
#         F2_prime        <-            F2
#     }else if (F3-F2>2.5){                   #F3 is a bit higher than F2
#         c3 <- (3.5-(F3-F2))/2
#         F2_prime        <-      (F2 +  c3*F3)          / (1+c3)
#     }else if (F4-F2>3.5 | F4-F3>F3-F2){     #F4 is high enough to be irrelevant
#         F2_prime        <-      (2/3)*F2 + (1/3)*F3 
#     }else{                                  #
#         F2_prime        <-                 (2/3)*F3 + (1/3)*F4
#     }
#     #print(schwartz)
#     if (units=='hertz'){
#         hertz(F2_prime)
...
# schwartz_F2_prime3 <- function(F2,F3,F4,units='bark'){
#     if (units=='hertz'){
#         F2 <- bark(F2)
#         F3 <- bark(F3)
#         F4 <- bark(F4)
...
#     #start with elsewhere
#     F2_prime <- (2/3)*F3 + (1/3)*F4

#     c3 <- (3.5-(F3-F2))/2


#     #F3 is high enough to be irrelevant
#     case1 <- F3-F2>3.5
#     F2_prime[case1] <- F2[case1] 

#     #F3 is a bit higher than F2
#     case2 <- F3-F2>2.5 & (!case1)
#     F2_prime[case2] <- (F2[case2] + c3[case2]*F3[case2]) / (1+c3[case2])

#     #F4 is high enough to be irrelevant
#     case3 <- F4-F2<=3.5 & (!case1) & (!case2)
#     F2_prime[case3] <- (2/3)*F2[case3] + (1/3)*F3[case3]
